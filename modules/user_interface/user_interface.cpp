    ...
    if( keyReleased != '\0' ) {
        ...
        if( alarmStateRead() && !systemBlockedStateRead() ) {
            ...
            if( keyReleased == 'C' ) {
                GasLog = !GasLog;
                 if (GasLog==true){
                    pcSerialComStringWrite( "Gas event logging enabled\r\n" );
                } else {
                    pcSerialComStringWrite( "Gas event logging disabled\r\n" );
                }
            }
            if( keyReleased == 'D' ) {
                TempLog = !TempLog;
                if (TempLog==true){
                    pcSerialComStringWrite( "Temperature event logging enabled\r\n" );
                } else {
                    pcSerialComStringWrite( "Temperature event logging disabled\r\n" );
                }
            }
            if( keyReleased == '*' ) {
                DisplayLog = !DisplayLog;
                if (DisplayLog==false){
                    pcSerialComStringWrite( "Displaying Recent log data stopped, press * to start\r\n" );
                }
            }
    }
                if (DisplayLog==true){
                    pcSerialComStringWrite( "Displaying Recent log data, press * to stop\r\n" );
                        char eventstring[500];
                    for(int i = 0; i < eventLogNumberOfStoredEvents();i++){
                        eventLogRead(i, eventstring);
                        pcSerialComStringWrite(eventstring);
                        float potreading;
                        potreading = 500 + Potentiometer.read()*3300;
                        delay(potreading);
                    }
